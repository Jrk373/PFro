...
Then we merge the Global Membership Roster with the PFro BH Claims records, attaching each behavioral health claim to its respective member information.
...
Then import the aggregated Health Choice VBP Quality Roster. This allows us to identify those members who have been served by a PFro, who have also been deemed eligible for a VBP measure. This is taken from the 7/21/23 report. 
...
# Assuming "dob" is in date format, if not convert it using as.Date() function
PFroSummary_Distinct_byProvider <- PFroSummary_Distinct_byProvider %>%
...
## Now entering the PFro Member Engagement Matrix 

|  |VBP|No VBP|
|--|--|--|
| PFro | VBP PFro | No VBP PFro |
| No PFro | VBP No PFro | No VBP No PFro |
...
# Assuming the data frames NAZMmbrshp and PFroBHClaims are already loaded

PFroEngagementMatrix <- data.frame(
  AHCCCSID = NAZMmbrshp$AHCCCSID,
  isPFro = ifelse(NAZMmbrshp$AHCCCSID %in% PFroSummary_Distinct_byProvider$AHCCCSID, "PFro", "NoPFro")
)

# Now add VBPQR
PFroEngagementMatrix <- data.frame(
  AHCCCSID = NAZMmbrshp$AHCCCSID,
  isPFro = ifelse(NAZMmbrshp$AHCCCSID %in% PFroBHClaims$AHCCCSID, "PFro", "NoPFro"),
  isVBP = ifelse(NAZMmbrshp$AHCCCSID %in% VBPQR$AHCCCSID, "VBP", "NoVBP")
)

# Create a summary table
summary_table <- table(PFroEngagementMatrix$isPFro, PFroEngagementMatrix$isVBP)

kable(summary_table)
```

```{r}
#| label: No PFro Summary
#| eval: true
#| include: true
#| echo: true
#| warning: false
...
# Rename the columns
colnames(summary_df) <- c("PFro", "VBP", "n")

# Define the custom colors
my_colors <- c("VBP" = "#FFA500", "NoVBP" = "#000080")

# Plot for NoPFro
ggplot(data = filter(summary_df, PFro == "NoPFro"), aes(x = VBP, y = n, fill = VBP)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = my_colors) +
  labs(title = "Summary of NoPFro Engagement",
       subtitle = "Distinct Members",
       x = "VBP",
       y = "Count",
       fill = "VBP") +
  theme_minimal() +
  theme(legend.position = "bottom") +
  facet_wrap(~ PFro, ncol = 1, scales = "free") +
...
ggplot(data = filter(summary_df, PFro == "PFro"), aes(x = VBP, y = n, fill = VBP)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = my_colors) +
  labs(title = "Summary of PFro Engagement",
       subtitle = "Distinct Members",
       x = "VBP",
       y = "Count",
       fill = "VBP") +
...
ggplot(data = summary_table, aes(x = fct_reorder(resCountyName, Count_of_Distinct_MemberID, .fun = sum), y = Count_of_Distinct_MemberID, fill = ProviderName)) +
  geom_bar(stat = "identity") +
...
Fig_PopPyramid <- ggplot(DemoTable_SexByAgeGroup, aes(x = n, y = reorder(Age_Group, n), fill = sex)) +
  ...
  scale_fill_manual(values = c(Female = "#5b3493", Male = "#434444")) +
...
Report <- "./data/output/"
Report_Filename <- "2023-07-26_Fig_PopPyramid.png"
